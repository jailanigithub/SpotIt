...
const CGFloat TOLTargetLightPadding = -4.f;
...
- (void)setup{
    ...
    self.glowColors = @[[UIColor colorWithRed:(74.0/255.0) green:(201.0/255.0) blue:(42.0/255.0) alpha:1.0],
                         [UIColor colorWithRed:(255.0/255.0) green:(128.0/255.0) blue:(0.0/255.0) alpha:1.0],
                         [UIColor colorWithRed:(255.0/255.0) green:(0.0/255.0) blue:(0.0/255.0) alpha:1.0]];
    //Blue green
    self.leftChannelLevel = 1.0f;
    self.rightChannelLevel = 1.0f;
}
...
- (UIColor *)colorForLightNum:(NSInteger)lightNum totalLights:(NSInteger)totalLights{
    ...
    CGFloat lightPercentage = (lightNum*100)/(CGFloat)totalLights;
//    NSLog(@"Light percentage: %f", lightPercentage);
    if(lightPercentage <= 50 && self.glowColors.count > 0){
        return [self.glowColors objectAtIndex:0];
    }else if (lightPercentage > 50 && lightPercentage <= 80 && self.glowColors.count > 1) {
        return [self.glowColors objectAtIndex:1];
    }else if(self.glowColors.count > 2){
        return [self.glowColors objectAtIndex:2];
    }
    return nil;
}
